...
@app.command()
def dialog(
    minimal: bool = typer.Option(False, "--minimal", help="极简版：仅截图标注，不解析流程图/视频"),
):
    """启动可视化对话框（截图标注、流程图展示、视频链接）。与 nanobot agent 共用同一配置与会话。"""
    from nanobot.ui.dialog_app import run_dialog
    run_dialog(minimal=minimal)
...
def web(
    ...
    """启动浏览器版对话界面（Web 页面，Markdown + 流程图渲染）。"""
    try:
        from nanobot.ui.web_app import run_web
    except ImportError as e:
        raise SystemExit("请先安装 Web 依赖: pip install nanobot-ai[web] 或 pip install flask") from e
