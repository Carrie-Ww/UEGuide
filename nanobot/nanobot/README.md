...
> - **MiniMax Coding Plan**: 编程套餐使用 **Anthropic 兼容接口**。在配置中：`agents.defaults.model` 设为 `"anthropic/MiniMax-M2.5"`，`providers.anthropic.apiKey` 填 Coding Plan Key，`providers.anthropic.apiBase` 设为 `"https://api.minimaxi.com/anthropic"`。详见 [MiniMax Coding Plan 文档](https://api.minimax.io)。
...
| `minimax` | LLM (MiniMax；Coding Plan 需用 anthropic + apiBase) | [api.minimax.io](https://api.minimax.io) |
